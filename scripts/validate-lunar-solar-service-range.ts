...
function verifySolarRoundTrip() {
  ...
  for (let time = start; time <= end; time += dayMs) {
    ...
    try {
      ...
      if (actualSolar.year !== solar.year || actualSolar.month !== solar.month || actualSolar.day !== solar.day) {
        addFailure({
...
if (failureCount > 0) {
  console.error(`Lunar/solar service range validation failed: ${failureCount} failure(s)`);
  ...
  console.log(`Lunar/solar service range validation passed for ${START_YEAR}-${END_YEAR}`);
  console.log("Round-trip failures: 0");
}
